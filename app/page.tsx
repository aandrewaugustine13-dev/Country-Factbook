...
export default function Home() {
  ...
  const loadCountries = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Live map is napping');
      ...
      const transformed = raw.map((c: any) => ({
        ...
        currency: Object.values(c.currencies || {}).map((cur: any) => `${cur.name} (${cur.symbol || ''})`.trim()).join(', ') || 'Unknown',
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setKidMode(!kidMode)} className={`px-5 py-2 rounded-full font-medium ${kidMode ? 'bg-purple-600 text-white' : 'bg-white border'}`}>
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        {loading && <div className="text-center py-20 text-2xl">🌍 Loading countries...</div>}
        {error && <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-xl mb-6 text-center">{error}</div>}
